(function(){
  ...
  function installAircraftTracking(){
    ...
    style.textContent=`
      ...
      .aircraft-gate-row:first-child{border-top:0}.aircraft-gate-row div{min-width:0}.aircraft-gate-row b{display:block;color:#eef7ff;font-size:11px}.aircraft-gate-value{display:flex;align-items:center;justify-content:flex-end;gap:10px;min-width:0}.aircraft-gate-terminal{display:inline-flex;align-items:center;justify-content:center;min-width:28px;padding:0;color:#eef7ff;font-size:18px;line-height:1;font-weight:1000;letter-spacing:.03em}.aircraft-gate-terminal.t1{color:#ff9d45}.aircraft-gate-terminal.t2{color:#61c9ff}.aircraft-gate-terminal.t3{color:#c4a7ff}.aircraft-gate-terminal.other{color:#cbd5df}
      .aircraft-gate-row span{display:block;margin-top:2px;color:#9fb0c5;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.aircraft-gate-row .aircraft-gate-terminal{font-size:18px;margin-top:0}
    ...
    gateForm.addEventListener("submit",async e=>{
      ...
      try{
        ...
        gateResult.innerHTML=`<div class="aircraft-gate-result-head"><strong>${escapeHtml(data.query)} 登機門<span class="aircraft-gate-route-inline">${escapeHtml(routes)}</span></strong><small>資料 ${escapeHtml(data.fetchedAt?.slice(11,16)||"--:--")} 更新</small></div>${matches.map(match=>{const gate=match.gate||"尚未公布";return `<div class="aircraft-gate-row"><div><b>${escapeHtml(match.direction)}</b><span>${escapeHtml(match.date)} ${escapeHtml(match.time)}${match.status?` · ${escapeHtml(match.status)}`:""}</span></div><div class="aircraft-gate-value"><span class="aircraft-gate-terminal ${terminalClass(match.terminal)}">${escapeHtml(match.terminal||"-")}</span><strong class="${match.gate?"":"is-empty"}">${escapeHtml(gate)}</strong></div></div>`}).join("")}`;
