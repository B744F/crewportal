(function(){
  ...
  function installAircraftTracking(){
    ...
    style.textContent=`
      ...
      .aircraft-gate-result-head strong{color:#dcefff;font-size:11px}.aircraft-gate-result-head small{white-space:nowrap}.aircraft-gate-route-inline{display:inline-block;margin-left:14px;color:#ffd400;font-size:12px;font-weight:1000;letter-spacing:.09em;white-space:nowrap}
      ...
      .aircraft-gate-row:first-child{border-top:0}.aircraft-gate-row div{min-width:0}.aircraft-gate-row b{display:block;color:#eef7ff;font-size:11px}.aircraft-gate-line{display:flex;align-items:center;gap:6px;margin-top:4px}.aircraft-gate-line>span:last-child{margin-top:0;color:#eef7ff;font-size:10px;font-weight:800;white-space:nowrap}.aircraft-gate-terminal{display:inline-flex;align-items:center;justify-content:center;min-width:25px;padding:0;color:#eef7ff;font-size:10px;font-weight:1000;letter-spacing:.03em}.aircraft-gate-terminal.t1{color:#ff9d45}.aircraft-gate-terminal.t2{color:#61c9ff}.aircraft-gate-terminal.t3{color:#c4a7ff}.aircraft-gate-terminal.other{color:#cbd5df}
      .aircraft-gate-row span{display:block;margin-top:2px;color:#9fb0c5;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    ...
    gateForm.addEventListener("submit",async e=>{
      ...
      try{
        ...
        gateResult.innerHTML=`<div class="aircraft-gate-result-head"><strong>${escapeHtml(data.query)} 登機門<span class="aircraft-gate-route-inline">${escapeHtml(routes)}</span></strong><small>資料 ${escapeHtml(data.fetchedAt?.slice(11,16)||"--:--")} 更新</small></div>${matches.map(match=>{const gate=match.gate||"尚未公布";return `<div class="aircraft-gate-row"><div><b><span class="aircraft-gate-line"><span class="aircraft-gate-terminal ${terminalClass(match.terminal)}">${escapeHtml(match.terminal||"-")}</span><span>${escapeHtml(match.direction)}</span></span></b><span>${escapeHtml(match.date)} ${escapeHtml(match.time)}${match.status?` · ${escapeHtml(match.status)}`:""}</span></div><strong class="${match.gate?"":"is-empty"}">${escapeHtml(gate)}</strong></div>`}).join("")}`;
