(function(){
  ...
  function installAircraftTracking(){
    ...
    style.textContent=`
      ...
      .aircraft-gate-row strong{min-width:54px;padding:5px 7px;text-align:center;background:#ffd21f;border:1px solid #ffea70;border-radius:4px;box-shadow:0 1px 0 rgba(0,0,0,.35),inset 0 -2px 0 rgba(104,72,0,.28);color:#08111b;font-size:18px;line-height:1;letter-spacing:.03em}.aircraft-gate-row strong.is-empty{color:#08111b;font-size:11px}.aircraft-gate-row strong.is-flown{background:#d93645;border-color:#ff7580;box-shadow:0 1px 0 rgba(0,0,0,.35),inset 0 -2px 0 rgba(112,0,12,.32);color:#fff;font-size:15px;letter-spacing:.02em}
    ...
    const renderFlightRow=(match,cargo=false,freshness="fresh")=>{const departed=flightHasDeparted(match),arrived=flightHasArrived(match),rawStatus=String(match.status||""),status=departed?"DEPARTED":rawStatus,position=cargo&&departed?"":(cargo?match.stand:match.gate),emptyLabel=cargo&&departed?"DEPARTED":cargo?"無資料":(rawStatus.includes("取消")||departed||arrived?"無資料":"未定"),displayDate=match.estimatedDate||match.date,displayTime=match.estimatedTime||match.time,timeChanged=match.estimatedTime&&match.estimatedTime!==match.time,statusMarkup=status?` · <em class="${departed?"aircraft-gate-status-flown":""}">${escapeHtml(status)}</em>`:"",timeMarkup=timeChanged?" · 預計":"",aircraftMarkup=cargo&&match.aircraftType?` · ${escapeHtml(match.aircraftType)}`:"",terminalMarkup=cargo?"":`<span class="aircraft-gate-terminal ${terminalClass(match.terminal)}">${escapeHtml(terminalCode(match.terminal))}</span>`,positionClass=position?"":"is-empty",flownClass=cargo&&departed?" is-flown":"";return `<div class="aircraft-gate-row${cargo?" cargo":""}"><div><b>${escapeHtml(directionLabel(match.direction))}</b><span>${escapeHtml(displayDate)} ${escapeHtml(displayTime)}${timeMarkup}${aircraftMarkup}${statusMarkup}</span></div><div class="aircraft-gate-value">${terminalMarkup}<strong class="${positionClass}${flownClass}">${escapeHtml(position||emptyLabel)}</strong></div></div>`};
