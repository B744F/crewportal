...
- 高鐵車次展開時排除已通過及所選車站，只顯示所選車站之後的停靠站與時間；車次旁 stops 數量同步更新。

## v8.2.91 — 2026-09-14

- 修正手機版 AIRPORT MRT 與 TAIWAN HSR 車站下拉選單箭頭在深色背景上不易辨識的問題，改用主題色自訂箭頭。

## v8.2.90 — 2026-09-14

- 修正手機版高鐵車次時間與車次文字重疊。
- 改用去背後的台灣高鐵 Logo，並在北上／南下標題補充往南港／往左營。
- Worker 2.8.83：同步正式站 Portal／Worker 版本資訊。

## v8.2.89 — 2026-09-14

- Worker 2.8.82：修正 Airport MRT edge cache 依 Origin 分流，避免本機測試的 CORS 回應被正式站重用。
...
- 新增可由鍵盤操作、點擊外部或按 `Esc` 關閉的雙語使用說明視窗。
- 說明內容涵蓋 ATIS、停車資訊、機場捷運與 Pacific HF 的用途、操作方式及資料解讀。
- 同步更新 Portal、Build 與靜態資產快取版本。
