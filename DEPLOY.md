# FlightDeck Crew Portal v8.1.0 部署
...
```json
{
  "portalVersion": "v8.1.0",
  "workerVersion": "2.8.3",
  "timetableParser": "structured-official"
}
```

航班登機門查詢使用 `/api/flight-gate?flight=CI100`，資料由 GitHub Actions 每 5 分鐘擷取桃園機場 ADIP 官方即時航班資料並更新快照。擷取失敗會改用 Cloudflare `/api/flight-gate-source`，再以 TDX 官方 Airport FIDS `/api/flight-gate-tdx-source` 備援；資料異常或當日完全沒有門位時會拒絕發布，API 會回傳資料年齡與 `freshness` 狀態。
