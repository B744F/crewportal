# FlightDeck Crew Portal v8.2.90
...
- Portal Version：v8.2.90
- Worker Version：2.8.83
...
- 「生活 / 工具」提供 `PSX NAV DATA`，下方顯示目前 AIRAC 生效日 `Cycle Effective 03-Sep-26`，並直接下載 Google Drive 的 `AIRAC.zip`。
- 公開航班查詢不再觸發 TDX Airport FIDS；TDX FIDS 僅保留給排程備援，避免使用者數增加時放大 API 用量。
- 登機門查詢信任 Worker 已完成今日資料篩選，避免瀏覽器日期格式差異將有效航班誤判為查無資料。
- 登機門查詢延長官方資料等待時間，並將航班資料逾時正確顯示為暫時無法取得，不再誤報為查無航班。
- 登機門與貨機坪查詢改為先顯示已完成的來源結果，官方即時更新於背景執行，避免慢來源阻塞畫面。
- 貨機查詢在貨機坪資料完成前，不再先顯示客機來源的「未定」中間狀態。
- SYSTEM STATUS 新增 Visitor Geography 連結，顯示各國旗幟、國家名稱與累計造訪次數。
- 不存在航班或官方來源逾時時，查詢會在總時限後明確結束，不會無限停留在查詢中。
- 查詢完成後不再被先前的慢速提示計時器覆蓋，避免「查無資料」又變回「仍在查詢中」。
